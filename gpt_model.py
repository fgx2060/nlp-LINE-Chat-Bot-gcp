...
def create_masks(input):
    ...
    input_mask = input!= 0  # 屏蔽掉填充詞（填充符的標號為0）
    input_mask = input_mask.unsqueeze(1)  # (batch_size, 1, max_len)
    input_mask = input_mask & subsequent_mask(input.size(-1)).type_as(input_mask.data)  # 統一成int形式再進行與操作，shape：(batch_size, max_len, max_len)
...
class Embeddings(nn.Module):
    ...
    def __init__(self, vocab_size, emb_dim, max_pos):
        ...
        self.embed = nn.Embedding(vocab_size, emb_dim)  # 文字標號轉emb詞向量
        self.pos_emb = nn.Embedding(max_pos, emb_dim)  # 位置編碼

    def forward(self, encoded_words):
        # 輸入形狀：[batch_size, max_len]
        max_len = encoded_words.size(1)
        pos = torch.arange(max_len, dtype=torch.long, device=device)
        pos = pos.unsqueeze(0).expand_as(encoded_words)  # [max_len] -> [batch_size, max_len] —— 0為填充符
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        mask = mask.unsqueeze(1).repeat(1, self.heads, 1, 1)  # attn_mask : [batch_size, n_heads, max_len, max_len]
   
        scores = scores.masked_fill(mask == 0, -1e9)    # (batch_size, h, max_len, max_len) masked_fill()函數主要用在transformer的注意力機制中，在時序任務中，主要是用來mask掉當前時刻後面時刻的序列信息(即不為0的用-1e9替換)
       
        weights = F.softmax(scores, dim=-1)  # (batch_size, h, max_len, max_len) 那麼前面被mask掉的（即-1e9的概率會很小很小，幾乎為0，實現了屏蔽的效果）
        ...
        context = context.permute(0,2,1,3).contiguous().view(context.shape[0], -1, self.heads * self.d_v)  # torch.contiguous()方法首先拷貝了一份張量在內存中的地址,否則直接view是不会重新創建一個的
        #(batch_size, max_len, h * d_v)
        interacted = self.concat(context)
        return interacted  # (batch_size, max_len, emb_dim)

# FeedForward的輸入是Multi-Head Attention的輸出做了殘差連接和Norm之後的數據，然后FeedForward做了兩次線性變換，为的是更加深入地提取特徵。
class FeedForward(nn.Module):
    # torch.nn.Linear的輸入和輸出的維度可以是任意的，默认對最後一維做全連接
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs):
        ...
        dec_self_attn_mask = create_masks(dec_inputs)  # [batch_size, max_len, max_len] 生成mask矩陣
